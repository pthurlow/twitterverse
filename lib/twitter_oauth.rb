...
class TwitterOauth
  ...
  # Twitter REST API Method: user show
  def user(user)
    response = access_token.get("/users/show.json?screen_name=#{user}")
    case response
    when Net::HTTPSuccess
      show_user=JSON.parse(response.body)
    else
      raise TwitterOauth::APIError
    end
    show_user
